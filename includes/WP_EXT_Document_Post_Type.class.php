...
/**
 * Class WP_EXT_Document_Post_Type
 * ------------------------------------------------------------------------------------------------------------------ */

class WP_EXT_Document_Post_Type extends WP_EXT_Document {
	...
	public function post_type() {
		$labels       = [
			'name'                  => _x( 'Документы', 'Document General Name', 'wp-ext-' . $this->domain_ID ),
			'singular_name'         => _x( 'Документ', 'Document Singular Name', 'wp-ext-' . $this->domain_ID ),
			'menu_name'             => __( 'Документы', 'wp-ext-' . $this->domain_ID ),
			'name_admin_bar'        => __( 'Документ', 'wp-ext-' . $this->domain_ID ),
		...
		$args         = [
			'label'               => __( 'Документ', 'wp-ext-' . $this->domain_ID ),
			'description'         => __( 'Document Description', 'wp-ext-' . $this->domain_ID ),
			'labels'              => $labels,
 ...
 *
 * @return WP_EXT_Document_Post_Type
 * ------------------------------------------------------------------------------------------------------------------ */

function WP_EXT_Document_Post_Type() {
	static $object;

	if ( null == $object ) {
		$object = new WP_EXT_Document_Post_Type;
	}
...
add_action( 'plugins_loaded', [ WP_EXT_Document_Post_Type(), 'run' ] );
